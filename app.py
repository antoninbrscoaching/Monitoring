...
export default function App() {
  ...
  function startDemo() {
    setDemoMode(true);
    const enriched = DEMO_ATHLETES.map(a => ({ ...a, _hoopers: DEMO_HOOPER[a.id], _sessions: DEMO_SESSIONS[a.id] }));
    setAthletes(enriched); setSelected(enriched[0]); setScreen("app");
  }

  function connectNolio() {
    const params = new URLSearchParams({ response_type: "code", client_id: config.clientId, redirect_uri: NOLIO_CONFIG.REDIRECT_URI });
    window.location.href = `${NOLIO_CONFIG.AUTH_URL}?${params}`;
  }

  function saveHooper(athlete, vals) {
    const update = list => [...list.filter(h => h.date !== vals.date), vals].sort((a, b) => a.date.localeCompare(b.date));
    setAthletes(prev => prev.map(a => a.id === athlete.id ? { ...a, _hoopers: update(a._hoopers || []) } : a));
    if (selected?.id === athlete.id) setSelected(a => ({ ...a, _hoopers: update(a._hoopers || []) }));
  }
